...
from knitpkg.core.exceptions import KnitPkgError, InvalidUsageError
...
def register(app):
    ...
    def init_project(
        ...
        try:
            ...
            console_awr.print(f"\n[bold]🚫 {str(e).replace('Invalid usage:','[magenta]Invalid usage:[/magenta]')}[/bold]")
            console_awr.print("")
            raise typer.Exit(code=1)
        
        except KnitPkgError as e:
            console_awr.print(f"\n[bold red]❌ Init failed:[/bold red] {e}")
            console_awr.print("")
            raise typer.Exit(code=1)
