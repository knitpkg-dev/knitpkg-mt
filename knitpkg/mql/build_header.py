...
def _format_define(name: str, value: Optional[str]) -> str:
    ...
    * anything else           →  ``#define NAME "text"``
    ...
    if _NUMERIC_RE.match(value.strip()):
        return f"#define {name:<40} {value.strip()}"

    escaped = value.replace("\\", "\\\\").replace('"', '\\"')
    return f'#define {name:<40} "{escaped}"'


def _make_include_guard(project_name: str) -> str:
    """
    Derive an include-guard symbol from the project name.

    Example: ``"my-project"`` → ``__KNITPKG_MY_PROJECT_MANIFEST__``
    """
    sanitized = re.sub(r'[^A-Za-z0-9]', '_', project_name).upper()
    return f"__KNITPKG_{sanitized}_MANIFEST__"
...
class ManifestHeaderGenerator:
    ...
    def _render(self, entries: list[tuple[str, Optional[str]]]) -> str:
        ...
        guard     = _make_include_guard(self._manifest.name)
