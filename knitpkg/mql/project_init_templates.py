...
//--------------------------------------------------------------------
// For packages with dependencies, use the Autocomplete support 
// to compile the headers and the Unit tests. To do so, use 'kp add'
// command to update your manifest with the required dependencies and 
// then run `kp autocomplete`. It will download all the dependencies 
// and make its source code available under knitpkg/autocomplete/ 
// directory. 
//
// Once dependencies are locally available, include the required 
// headers from that folder annotated with @knitpkg:wired directive 
// as in the example below:
//
// #include "{{autocomplete_path_prefix}}autocomplete/knitpkg/include/douglasrechia/logger/Logger.mqh" /* @knitpkg:wired */
//
// When this package is installed as a dependency in another
// project, KnitPkg automatically resolves the annotated includes to
// the real (external) headers.
//--------------------------------------------------------------------
...
1. If your package has dependencies on other packages:
   - Add them to the dependencies section in knitpkg.yaml, you can use 'kp add'
   - Run 'kp autocomplete' to resolve dependencies for the Unit tests and 
...
For detailed documentation, visit: https://docs.knitpkg.dev/user-guide/creating-packages/
...
//
// Run `kp install` after you add your headers.
//--------------------------------------------------------------------
...
`kp init --type expert`):
  ...
  <ProjectName>/
    ...
    build/               # Unversioned compile-time directives
  src/
...
When **flat** mode is used (recommended), `kp install` will generate a 
...
package layout) after you run `kp install`. 
...
* Add a dependency: Use 'kp add' to add a dependency to your manifest. 
Alternatively, you can edit `knitpkg.yaml` manifest file and add any package you 
want under the `dependencies` section. For example:

  dependencies:
    # others dependencies...
    '@knitpkg.dev/knitpkg-mt-bar': ^1.0.0
...
* Update a dependency: in the case you need to update a dependency, just
  update the version in `knitpkg.yaml` and run `kp install` again to
...
2. **Version control** - commit all the files respecting the default 
   `.gitignore` (automatically generated by `kp init`). If you prefer, 
   ...
   `kp install` will add the resolved dependencies into one of 
   those directories.

4. **Register** - when you are ready, push the repository to a Git host.

For the full KnitPkg documentation visit: https://docs.knitpkg.dev/user-guide/creating-projects/
...
================================================================================
                         QUICK START - CREATE YOUR INDICATOR
================================================================================
...
2. Attach the indicator to a chart
   ...
   After a successful compilation you will have an `.ex5` (or `.ex4`)
   file along with your src/<ProjectName>.mq5 (or `.mq4`) in the MetaTrader 
   `Indicators` folder. Attach it to a chart and start testing.
...
================================================================================
                         QUICK START - CREATE YOUR SCRIPT
================================================================================
...
2. Run the Script
   ...
   After a successful compilation you will have an `.ex5` (or `.ex4`)
   file along with your src/<ProjectName>.mq5 (or `.mq4`) in the MetaTrader 
   `Scripts` folder. Attach it to a chart and start testing.
...
================================================================================
                         QUICK START - CREATE YOUR LIBRARY
================================================================================
...
4. **Write your trading logic**  
   ...
   Implement the usual Library callbacks inside the skeleton.  You can also create 
   additional helper headers in `src/` and include them with normal MQL #include
   directives.
...
2. Run the EA
   ...
   After a successful compilation you will have an `.ex5` (or `.ex4`)
   file along with your src/<ProjectName>.mq5 (or `.mq4`) in the MetaTrader 
   `Libraries` folder. Attach it to a chart and start testing.
...
================================================================================
                         QUICK START - CREATE YOUR SERVICE
================================================================================
...
4. **Write your trading logic**  
   ...
   Implement the usual Service callbacks inside the skeleton.  You can also 
   create additional helper headers in `src/` and include them with normal MQL 
   #include directives.
...
2. Run the Service
   ...
   After a successful compilation you will have an `.ex5` (or `.ex4`)
   file along with your src/<ProjectName>.mq5 (or `.mq4`) in the MetaTrader 
   `Services` folder. Right-click on it and click on "Start".
