...
class ProjectInitializer(ConsoleAware):
    ...
    def prompt_version(self, version: Optional[str]) -> None:
        ...
        if not match:
            self.log("[red]Error: Version must follow SemVer format (e.g., 1.0.0, 2.1.3-beta.1)[/red]")
            raise typer.Exit(code=1)
        
        major_version = int(match.group('major'))
        if major_version == 0:
            self.log("[red]Error: Major version 0 is not supported. Please use version 1.0.0 or higher.[/red]")
            raise typer.Exit(code=1)
    ...
    def handle_existing_directory(self) -> None:
        ...
        if project_root.exists():
            self.print(
                f"[yellow]Warning: Directory '{project_root}' already exists.[/yellow]"
            )
            ...
            if existing_files:
                ...
                if not Confirm.ask("Project files will be overwritten. Continue?", default=False):
                    self.print("[red]Project initialization aborted by user.[/red]")
                    raise typer.Exit(code=1)
        ...
        try:
            ...
            if self.git_init:
                try:
                    ...
                    self.print(f"[red]Error initializing Git: {e}[/red]")
    ...
    def run(
        ...
        if not self.display_summary_and_confirm():
            self.print("[red]Project creation cancelled.[/red]")
            raise typer.Exit()
