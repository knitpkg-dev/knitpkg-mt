...
!!! warning "Third-party antivirus false positive (kp.exe)"
    Some users have reported that certain **third-party antivirus** products are flagging `kp.exe` as a threat due to a **false positive**. This does **not** happen with **Windows Defender** (the default Windows antivirus).

    We have already reported this issue to **Norton Antivirus** and are working on additional measures to mitigate it.
