...
## Autocomplete + @knitpkg:wired *(v1.1.0+, recommended)*

Instead of using `autocomplete.mqh` + comment-based directives, you can use the `@knitpkg:wired` approach: write a **real MQL `#include`** pointing directly to the header inside `knitpkg/autocomplete/knitpkg/include/`, and annotate it with `/* @knitpkg:wired */` on the same line.

The equivalent `Cross.mqh` using this approach:

```mql5 linenums="1" hl_lines="7"
//+------------------------------------------------------------------+
//|                                                       Cross.mqh  |
//|                                                Package barhelper |
//|                                      Organization: douglasrechia |
...
Compared to @knitpkg:include:

- No `autocomplete.mqh` include needed — MetaEditor resolves the path directly.
- Only `TimeSeries.mqh` is pulled in, so IntelliSense is not polluted with unrelated symbols.
- The file reads like standard MQL with no comment-based directives.

During `kp install`, KnitPkg detects the `/* @knitpkg:wired */` annotation and rewrites the path to point to the real installed header.

!!! note
    The headers under `knitpkg/autocomplete/knitpkg/include/` are generated by `kp autocomplete`. Run it whenever your dependencies change.

!!! note
    As with any approach, run `kp checkinstall` to validate that all wired includes will resolve correctly when the package is installed as a dependency.
