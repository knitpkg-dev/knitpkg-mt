...
## Option 1: Autocomplete + @knitpkg:wired *(v1.1.0+, recommended)*

Now that the dependency is added, let's generate the headers under `knitpkg/autocomplete/knitpkg/include/` by running:

```bash
kp autocomplete
```

With the `@knitpkg:wired` approach, you write a **real MQL `#include`** pointing directly to the header inside `knitpkg/autocomplete/knitpkg/include/`, relative to the location of the including header, and annotate it with `/* @knitpkg:wired */` on the same line.

Open `knitpkg/include/douglasrechia/barhelper/Header.mqh`, rename it to `Cross.mqh`, and implement the `CrossUp` function. The complete file looks like this:
...
By this way, whenever `barhelper` is installed as a dependency in another project,  `kp install` detects the `/* @knitpkg:wired */` annotation and rewrites the path to point to the real installed header.
...
!!! note
    Run `kp checkinstall` to validate that all wired includes will resolve correctly when the package is installed as a dependency.

---

## Option 2: Autocomplete + @knitpkg:include

The classic approach uses `autocomplete.mqh` for IntelliSense support combined with `@knitpkg:include` comment-based directives.

Run `kp autocomplete` to generate `knitpkg/autocomplete/autocomplete.mqh`. Open `knitpkg/include/douglasrechia/barhelper/Header.mqh`, uncomment the line that includes `autocomplete.mqh`, and rename the file to `Cross.mqh`.
