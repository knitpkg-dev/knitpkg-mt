...
### Option 1: autocomplete.mqh + @knitpkg:include
...
### Option 2: @knitpkg:wired *(v1.1.0+, recommended)*

Introduced in KnitPkg v1.1.0, the `@knitpkg:wired` directive is the recommended approach. Instead of using comment-based directives, dependencies are declared as **normal MQL `#include` statements** pointing to the autocomplete folder, marked with the `@knitpkg:wired` annotation:
...
This approach is more natural to MQL developers because:
